<template>
    <div class="card">
      ...
      <div class="card-body">
        <div class="team">
          <h3 class="game-score">{{ homeTeamScore }}</h3>
          <h3 class="team-name">{{ game.home_team }}</h3>
          <h4>(Home)</h4>
        </div>
        <div><h2>VS</h2></div>
        <div class="team">
          <h3 class="game-score">{{ awayTeamScore }}</h3>
  ...
  export default {
    ...
    computed: {
        ...
        homeTeamScore() {
            ...
            const scoreObject = this.game.scores.find(score => score.name === this.game.home_team);
        ...
        awayTeamScore() {
            if (this.game && this.game.scores) {
            const scoreObject = this.game.scores.find(score => score.name === this.game.away_team);
            return scoreObject ? scoreObject.score : '-';
            }
            return '-';
        }
